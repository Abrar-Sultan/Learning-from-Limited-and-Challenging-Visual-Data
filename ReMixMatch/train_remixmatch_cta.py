...
from remixmatch_dataset import get_train_dataloader_labeled, get_train_dataloader_unlabeled, get_val_dataloader_labeled, get_cta_probe_dataloader
from ctaugment import CTAugment
from utils import mixup_one, interleave, soft_cross_entropy_loss, ramp_up, estimate_prior_distribution, rotation_loss, cycle_loader, MovingAverage
...
@torch.no_grad()
def update_cta(cta, ema_model, probe_batch):
    '''
    this code is implemented with the help of: https://github.com/vfdev-5/FixMatch-pytorch/tree/master
    '''
    ema_model.eval()
    # getting data
    x = probe_batch["image"].to(device)
    y = probe_batch["target"].to(device)
    policy_json = probe_batch["policy"]
    ...
    # per-sample proximity and rate update
    for i in range(x.size(0)):
        error = y_proba[i].clone()
        error[y[i]] -= 1.0
        error = error.abs().sum().item()               
        p = 1.0 - 0.5 * error              
        policy = cta.deserialize(policy_json[i]) 
        cta.update_rates(policy, p)


# training function
def training_step(model, ema_model, train_data_loader_labeled, train_data_loader_unlabeled, optimizer, k, T, 
                  p_data, p_model, num_classes, alpha, lambda_u, lambda_u1, lambda_r, warm_up, ema_decay, rotation_head, total_iter, 
                  cta=None, cta_probe_iter=None, cta_update_every=64, use_dm=True):
    ...
    for index, batch in enumerate(progress_bar):
        ...
        x_u = torch.cat([x_u_weak] + [x_u_strong_k[:, i] for i in range(k)], dim=0)  # [B*(K+1), C, H, W]
        u_pseudo_l = y_pseudo 
        ...
        u_labels = u_pseudo_l.view(k+1, batch_size, num_classes)   # [(K+1), B, C]
        ...
        # de-interleave logits to get back original order
        ...
        # ramp up
        ramp = ramp_up(global_step, warm_up, total_iter)
        lambda_u = ramp * base_lambda_u
        lambda_u1 = ramp * base_lambda_u1
        ...
        # update cta rate
        if cta:
            if global_step % cta_update_every == 0:
                probe_batch = next(cta_probe_iter)
                update_cta(cta, ema_model, probe_batch)
    ...
    print(f"  Avg Total Loss compare: {avg_total_loss_com:.4f}")
...
def model_training(args):
    ...
    train_unlabel_dataloader =  get_train_dataloader_unlabeled(data_path=args.train_path_unlabel, batch_size=args.batch_size, k=args.k, 
                                                              total_image=num_sample, use_cta=args.use_cta, cta=cta)
    val_dataloader = get_val_dataloader_labeled(data_path=args.val_path, batch_size=args.batch_size)
    cta_probe_loader = get_cta_probe_dataloader(data_path=args.train_path_label, batch_size=args.batch_size, cta=cta, total_image=num_sample)
    cta_probe_iter = cycle_loader(cta_probe_loader) 
    ...
    ema_model = get_model(args.model_name, args.num_class).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        total_training_loss, train_loss = training_step(model, ema_model, train_label_dataloader, train_unlabel_dataloader, optimizer, args.k, args.T,
                                    ...
                                    args.warm_up, args.ema_decay, rotation_head, total_iter, cta, cta_probe_iter, args.cta_update, args.use_dm)
        ...
        if val_accuracy > best_val_accuracy:
            ...
            checkpoint_path = os.path.join(args.save_dir, f'ReMixMatch_{args.model_name}_best_model_1_pct_cta_k2_cifar100.pt')
    ...
    plt.legend()
    plt.savefig('./plots/remixmatch_loss_curve_1_pct_cta_k2_cifar100', dpi=300)
    plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cta-update', type=int, default=64, help='iterations between CTA rate updates')
    parser.add_argument('--use-cta', type=bool, default=True)
